...
def editprofile(request, username):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            img = request.FILES['imageB']
            if img.size / 1024 > 700:
                return HttpResponse(
                    {"message": "El tamaño de la imagen debe ser menor que900 X 1200 Pixel, Sube de nuevo.", })
            current_avatar = user.imageB
            cropped_avatar = crop_image(current_avatar, img, request.POST, user.id)
            user.imageB = cropped_avatar
            user.save()
            # return redirect('feed')
    ...
    return render(request, 'social/register.html',
                  {'form': form, 'formName': formName, 'title': 'Editar Usuario', 'boton': 'Guardar'})
